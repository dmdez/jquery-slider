(function($) {
	...
	$.easing.__SlideIn = function (x, t, b, c, d) {
    ...
    $.slider = function(element, options) {
		...
		var actionEvent = ('ontouchstart' in window) ? "click" : slider.settings.event;
		...
		function setPosition(elem, idx) {
			...
			$elementLi.removeClass('slide-open').each(function(i) {
				...
				if ( idx == $(this).index() ) 
					slider.settings.beforeOpen(this);
				...
				$(this)
					...
					.animate(animateConfig, slider.settings.speed, slider.settings.easing, function() {
						if ( idx == $(this).index() ) {
							...
							slider.settings.afterOpen(this);
						}
					});
			});
			activeIndex = idx;
